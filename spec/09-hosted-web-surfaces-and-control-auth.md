...
- **Google** accepts Project-owned key, display name, client ID, write-only secret, and managed-profile choice while showing the exact server-owned issuer, fixed scopes, consent behavior, and callback guidance as read-only policy. No issuer, endpoint, scope, consent, or capability override enters browser state or the request.
- **Custom OIDC** accepts a canonical issuer and provides an explicit authenticated preflight before create. It also shows the current Project egress mode/revision: all canonical HTTPS origins are allowed by default, while exact-origin restriction is recommended. Changing issuer, provider variant, or policy invalidates the displayed result. The result shows only canonical issuer, admitted endpoint origins, fixed OwlAuth scopes/capabilities, and safe outcome classes. Operators can copy the normalized origins into the separate revision-fenced exact-origin policy workflow, then run preflight again. Create remains available only from the current form values and the server repeats validation; the browser never submits a preflight capability as authorization.
- **GitHub** uses the existing fixed named profile and never exposes managed-profile controls.
...
19. Google onboarding derives and rejects overrides for its exact server policy, custom OIDC preflight carries no secret or commit authority and is repeated before persistence, changing variants/issuer clears preflight and secret state, GitHub remains login-only, and Hosted renders only the snapshotted closed kind with local assets and bounded text.
