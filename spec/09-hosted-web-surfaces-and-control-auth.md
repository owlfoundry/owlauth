...
| Route class | Purpose |
| --- | --- |
| `<runtime-base>/auth/interactions/{opaque_handle}` | render or continue one bounded Project/Application login interaction |
| `<runtime-base>/auth/assets/{fingerprinted_asset}` | serve credential-free, version-matched hosted UI assets |
| `<runtime-base>/v1/projects/{project_id}/auth/*` | existing Runtime Project Auth API, including start, callback, handoff, session, and logout operations |
...
A hosted interaction is created or resumed only from authoritative login-transaction state. The page derives Project branding, the transaction-bound active provider, Application display metadata, and safe next actions from bounded public Runtime configuration. Caller query parameters cannot replace the stored Project, Application, provider assignment, browser binding, PKCE challenge, callback, or redirect.

After successful upstream authentication, OwlAuth resolves the Project user and creates the one-use handoff described by spec 03. Navigation returns only to the exact Application redirect stored in the transaction. The return carries only the short-lived, one-use, Application/redirect/PKCE-bound handoff material permitted by spec 03; provider tokens, the operator key, refresh credentials, internal errors, and arbitrary caller-supplied redirect targets are never rendered or forwarded.

Hosted pages may provide bound-provider presentation, login progress, safe local error/restart, browser-session reuse, and logout/confirmation interactions required by Runtime policy. A future provider picker requires an explicit Runtime transaction contract and cannot trust an arbitrary page value. They MUST NOT expose Project administration, cross-Project discovery, raw user directories, provider secrets, Control endpoint discovery, or Console links containing authority.
...
3. every hosted interaction is recovered from authoritative Project/Application/login-transaction state and rejects caller attempts to replace provider, callback, redirect, browser binding, or PKCE values;
4. successful hosted authentication returns only to the exact stored Application redirect with the bounded one-use handoff; safe error/restart paths cannot become open redirects;
