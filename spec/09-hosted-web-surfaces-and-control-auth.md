...
Auth and Control retain distinct listeners, routers, middleware, credentials, and transport budgets. Inside Auth, Runtime and the JSON-only Server API retain distinct routers, state, authentication, pools, and readiness inputs; Server API has no browser surface or CORS, and spec 13 owns its paths and Project server-key rules. Their configured external base URLs MAY use different origins, such as `https://auth.example.com/` and `https://admin.example.com/`, or one origin with disjoint non-root path prefixes, such as Auth at `https://auth.example.com/auth/` and Control at `https://auth.example.com/admin/`. Distinct origins are RECOMMENDED because they preserve an additional browser-origin/XSS boundary around the in-memory deployment operator key.
...
The Runtime and Server API surfaces MUST NOT mount the Control-relative `console/` tree, Control assets, Control OpenAPI, or Control API routes. An unauthenticated Console shell contains no deployment data, Project identifiers, topology, version detail beyond ordinary static asset metadata, or credential-derived state.

## Hosted Authentication UI

The Hosted Authentication UI is present on the Runtime surface of the Auth listener in `all` and `auth` modes. It is Project/Application interaction UI, not the downstream Application itself and not a general OAuth authorization endpoint.

A representative Auth-base-relative Runtime partition is:

| Route class                                                             | Purpose                                                                                                                                                                                                                                                         |
| ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<auth-base>/auth/interactions/{opaque_handle}`                         | first top-level GET conditionally binds one Runtime browser/CSRF context, then renders or continues one bounded Project/Application interaction                                                                                                                 |
| `<auth-base>/auth/browser-logout/{opaque_preparation}`                  | render the no-store confirmation for one access-token-prepared Project browser logout; mutation requires matching cookie plus same-origin CSRF POST                                                                                                             |
| `<auth-base>/auth/email/confirm/{challenge_id}#proof=<opaque_proof>`    | load a no-store transfer confirmation view, remove the fragment proof from history, and consume only through the exact same-origin `POST /v1/projects/{project_id}/auth/email/magic/confirm`                                                                    |
| `<auth-base>/auth/managed-reauthorizations/{opaque_interaction}`        | bind and render one Control-created exact managed-connection reauthorization; explicit provider start and callback may replace only its fenced renewable credential generation and never issue Application credentials                                          |
| `<auth-base>/auth/identity-mutations/{opaque_intent}`                   | bind and render only one Control-created identity-mutation proof ceremony; proof completion may attach one server-side slot receipt, and only a separate explicit final POST may mark all-current slots ready; neither can mutate identity or issue credentials |
| `<auth-base>/auth/assets/{fingerprinted_asset}`                         | serve credential-free, version-matched hosted UI assets                                                                                                                                                                                                         |
| `<auth-base>/projects/{project_public_id}/auth/callback/{provider_key}` | stable exact upstream provider callback; no alias or generic fallback                                                                                                                                                                                           |
| `<auth-base>/v1/projects/{project_id}/auth/*`                           | Runtime Project Auth API, including generic login start, transaction method selection, method-specific proof, handoff, session, and logout operations                                                                                                           |

Spec 05 is the stable Runtime path and release-ledger authority. These interaction paths stay outside the Control base and cannot use a generic fallback over `v1/*` or provider callback routes; renamed routes have no compatibility alias.
...
Hosted pages may provide admitted-method selection, selected-provider presentation, login progress, safe local error/restart, and logout interactions. When current policy and a valid hardened cookie permit Project browser-session reuse, the page may separately show a privacy-conservative generic **Continue with current session** confirmation. V1 displays no account, email, avatar, or user hint; introducing one requires a separately reviewed privacy capability. Its POST supplies only CSRF/expected transaction revision; Runtime derives and revalidates the exact same-Project session/user/auth age and atomically competes with method selection on `awaiting_method_selection`. Page input cannot name a session/user or silently reuse one. The implemented picker is only a view over the explicit transaction selection contract and never trusts an arbitrary page value. They MUST NOT expose Project administration, cross-Project discovery, raw user directories, provider secrets, Control endpoint discovery, or Console links containing authority.

Hosted HTML, interaction responses, and error pages use `Cache-Control: no-store`, restrictive CSP/framing/referrer/permissions policies, and no third-party executable assets. Opaque interaction handles are redacted from access logs, telemetry, error reports, and referrers. Runtime cookies remain `Secure`, `HttpOnly`, host-only where possible, and scoped to the narrowest configured Auth path. Hosted pages treat branding/profile/error content as untrusted and do not load caller-controlled remote scripts, styles, fonts, images, or navigation URLs.
...
- `OWLAUTH_CONTROL_API_KEY` is required whenever the Control listener or remote HTTP MCP endpoint is enabled. Startup fails before binding those surfaces when it is absent, empty, malformed, or below the documented minimum strength. Auth mode does not require it.
...
- Authentication failure returns the stable non-enumerating Control unauthorized response. Connection, request-shape, deadline, and concurrency limits apply before expensive application work.
...
The Console sends the operator key only to routes under its configured Control base URL. It does not send or proxy it to any Auth route, provider, analytics, error-reporting, font, image, or third-party origin. API responses and rendered state are cleared when the Console is locked or authentication fails.

### One-time Project server-key reveal

Project server-key administration is an ordinary operator-authenticated Control workflow. The create dialog accepts only a bounded label, then displays the generated credential exactly once from the original successful response. The credential is kept only in a dedicated ref and imperative code node owned by that active dialog—not React state—and is never inserted into a reusable form default, global query cache, route state, notification history, browser storage, URL, analytics, or error report.
...
- Except for the one-time Project server-key create result defined above, the Console never renders provider secret bytes, API keys, private keys, session/refresh credentials, or arbitrary backend/vendor errors. Secret-setting workflows accept write-only input and display only committed safe metadata.
...
Every Console action preserves the Control contract's expected revision, idempotency, confirmation, command validation, bounded pagination, and safe error semantics. High-impact actions display the exact safe target and require explicit interaction, but UI confirmation never replaces server authorization or transactional invariants.

Console routing below the Control-base-relative `console/` path MAY use client-side navigation. A direct navigation to a Console route returns the shell without intercepting Control-base-relative `v1/*`, health, the configured Runtime base, hosted interactions, or provider callback routes. V1 switches Projects by returning to the Projects directory and selecting another Project; it has no in-page switcher or cross-Project browser cache. Current Project authority comes only from the URL and active page state, with stale-response and unsaved-change guards.
...
Provider client secrets remain only in the active password control and one short-lived request object. Variant changes, preflight, submit completion, navigation, lock, and authentication failure clear every secret copy; no result repopulates it. Preflight never accepts a secret. Provider editing exposes only display name, client ID, and an optional new client secret: an empty secret performs a metadata-only provider-revision compare-and-swap, while a non-empty secret invokes the idempotent protected-generation replacement command and discards the submitted bytes. Provider key, kind, issuer, and callback identity remain read-only. A durable unfinished replacement is exposed only as safe pending state; the operator can re-enter the write-only secret to reconcile the stored target metadata or explicitly abandon the pending generation under the unchanged Provider revision. Closing or reloading the Console cannot make the Provider permanently unrecoverable, and abandon never publishes the pending bytes. Revision conflicts require an explicit refresh and re-entry rather than automatic mutation replay. Egress failures are presented as actions to update the Project exact-origin policy or provider metadata without showing remote bodies, endpoint paths, or secret references. Policy editing requires the observed policy revision and warns that tightening can immediately make existing custom providers unavailable. The Console explains that `allow_all` trusts provider destinations chosen by the self-hosting operator and that exact origins are an optional least-privilege restriction.
...
Accessibility and keyboard operation are release requirements. Responsive layout SHOULD support ordinary desktop and tablet administration; a separate mobile application is not part of this surface. Chromium and Firefox are the current qualified Hosted UI and Console baseline. WebKit and Safari are not declared supported until the same secure real-server end-to-end gate exists.
...
- direct PostgreSQL/key-provider access from the browser;
- a second Console-only API or server-side business implementation;
- Console assets on the Auth listener;
...
06. `all` and `control` fail closed without valid `OWLAUTH_CONTROL_API_KEY`; `auth` starts without loading it;
...
15. security tests cover DOM injection, malicious URLs, oversized values, unauthorized responses, capacity failures, redirect abuse, cross-plane route confusion, and secret redaction;
