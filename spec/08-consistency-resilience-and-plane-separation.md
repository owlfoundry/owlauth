...
OwlAuth uses one PostgreSQL authority and one shared Project-domain model across Runtime, Client, and Control. A successful security-state mutation means the corresponding PostgreSQL transaction committed. Redis publication, local cache refresh, telemetry export, and remote observations are not part of commit acknowledgment.
...
- Project client-key revocation affects the next Client request, while lifecycle-neutral coarsened usage telemetry cannot race it into a conflict.
...
| Dependency/failure                                                                           | Runtime behavior                                                                                                                                                                    | Client behavior                                                                                                       | Control behavior                                                                                                      | Correctness effect                                                                                                          |
| -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
...
| Project client-key digest version unavailable or inventory/readiness stale                   | unaffected; Runtime never loads the ring                                                                                                                                            | readiness fails when any active key references an unavailable version; requests never try another version             | creation under a new version remains disabled until required Client readiness is authoritative                        | no valid active key silently becomes unverifiable and no fallback digest authority                                          |
...
| Control unavailable or restarting for operator-key rotation                                  | continues in split-process topology; redundant `all` rollout may preserve capacity; single-instance `all` restart interrupts Runtime too                                            | continues in split-process topology; single-instance `all` restart interrupts Client too                              | unavailable                                                                                                           | no Runtime/Client-to-Control dependency or credential crossover; process topology still determines restart availability     |
| Runtime unavailable                                                                          | unavailable                                                                                                                                                                         | directory reads continue; introspection fails closed if required token verification material/authority is unavailable | Control remains subject to dependencies                                                                               | Client has no Runtime RPC dependency and never substitutes an offline guess                                                 |
| Client unavailable                                                                           | Runtime continues                                                                                                                                                                   | unavailable                                                                                                           | Control continues subject to dependencies                                                                             | Runtime and Control do not accept Project client keys or proxy Client requests                                              |
| Invalidation lost                                                                            | stale public cache ignored at authoritative decision                                                                                                                                | v1 Client reads query PostgreSQL on every request; no directory-response cache exists                                 | committed mutation remains                                                                                            | derived Runtime staleness only, no stale Client allow                                                                       |
...
Runtime, Client, and Control have separate listeners, middleware, connection/concurrency budgets, and PostgreSQL pool quotas. Shared process memory does not mean shared admission queues.
...
6. Client user/projection reads;
7. Control list/audit queries.

Per-Project fairness prevents one Project's provider outage, login spike, Client read load, or audit query from consuming every Runtime/Client/Control resource. Priority/fairness never bypasses Project qualification or transaction ordering.
...
flowchart LR
    ...
    Backend[Customer backend ingress] --> BL[Client listener]
...
Combined mode preserves all three listeners, credentials, routers, pools, readiness surfaces, and admission budgets while application calls and Project transactions remain in one process.
...
flowchart LR
    ...
    Backend[Customer backend ingress] --> BP1[Client process]
    Backend --> BP2[Client process]
    Private[Private/admin ingress] --> CP[Control process]
    RP1 --> PG[(Shared PostgreSQL + protected material)]
    RP2 --> PG
    BP1 --> PG
    BP2 --> PG
    CP --> PG
    ...
    BP2 --> Redis
    CP --> Redis
    RP1 --> KP[Runtime signer / secret opener]
    RP2 --> KP
    BP1 --> KPV[Client key-digest verifier]
    BP2 --> KPV
    CP --> KPC[Control provisioner / secret sealer + client-key issuer]
```

Split mode uses the same official binary—or the same deployment-specific statically composed custom binary—schema, application/domain modules, and provider SPI. There is no Runtime/Client-to-Control RPC, dynamic plugin discovery, duplicate domain policy, or separate authoritative database. Each process receives only the role-specific credentials/provider capabilities required by its mode; bundled-provider replicas that need signer/sealer/opener authority receive the exact same static custody root. Runtime and Client observe Control changes through PostgreSQL; Redis only reduces existing Runtime derived-cache latency and coordinates non-authoritative admission.

Planes may use different serving login credentials, database roles, and pools only against the same configured PostgreSQL server/database authority. Matching schema histories on independent targets are insufficient and such configuration is invalid. Schema preparation uses the separately authorized migration capability in spec 04 rather than DDL privileges in serving pools. Each selected plane becomes ready independently only after its pool, exact capability inventory, and route-local dependencies pass; `all` does not report aggregate readiness while any selected listener is unready.

## Conditions for physical plane separation

Split-process topology is justified only by concrete isolation needs:

- materially different Runtime/Client/Control scaling or resource quotas;
- private Client or Control network placement;
