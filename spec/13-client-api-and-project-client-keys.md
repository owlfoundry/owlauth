# 13 — Project Client API and customer caller boundaries
...
2. customer backends use the Project-scoped Client Plane and Project client keys;
...
The Client API gives a trusted customer backend a bounded Project user read model and authoritative online access-token introspection. It does not grant Project administration, expose upstream-provider credentials, or turn OwlAuth into a SaaS framework. OwlAuth publishes a complete Client OpenAPI document; customers own generated clients, business repositories, BFF/framework integration, authorization, billing, organizations, and application-specific user metadata.

Existing TypeScript, Python, and Rust SDKs remain Project Auth Runtime clients. They do not accept Project client keys or wrap the Client API.
...
| Customer backend, server-side BFF module, generated OpenAPI client | Client               | one active Project client key                                                            | fixed read-only Client API authority for exactly one active Project             |
...
- a Project client key is not a Control credential, Runtime publishable identifier, Project access token, or refresh token;
- the operator key is never accepted by Client or Runtime;
- a publishable identifier and end-user credential never authorize Client or Control;
- a Project client key for Project A never identifies, enumerates, or authorizes Project B.

## Three-listener architecture

```mermaid
flowchart LR
    Admin[OwlAuth operator] --> Control[Control listener]
    Backend[Customer backend] --> Client[Client listener]
    Frontend[Customer frontend and end user] --> Runtime[Runtime listener]

    subgraph Server[One owlauth-server process]
        Control --> CAdapter[Control adapters]
        Client --> BAdapter[Client adapters]
        Runtime --> RAdapter[Runtime and Hosted adapters]
...
Runtime, Client, and Control have distinct internal listeners and routers even in `--plane=all`. They have independent bind/external URLs, trusted-proxy settings, TLS policy, request bounds, timeouts, connection budgets, admission dimensions, metrics, and readiness.

Client serves JSON only. It serves no HTML, cookies, redirects, service worker, permissive CORS, or credentialed browser response. Client CORS is deny-by-default with no v1 allowlist because Project client keys must never enter browser code. A reverse proxy may expose distinct origins or disjoint non-root paths, but it must preserve all three internal listener boundaries. Host- or path-routing on one untrusted listener is not a substitute.

Composition modes are `all`, `runtime`, `client`, and `control`. Each role receives only the configuration and private capabilities it requires. Control creates/revokes Client credentials; Client verifies and uses them; Runtime receives neither capability.

## Project client key model

A Project may have multiple independently named active keys so an operator can overlap deployment and rotation. V1 keys all grant the same fixed, read-only Client API authority for their exact Project. OwlAuth does not expose decorative scopes that cannot yet constrain distinct write capabilities.
...
```text
owl_client_v1.<public-key-id>.<43-character-base64url-secret>
```

The public key ID is exactly 16 independent CSPRNG bytes encoded as 22 canonical unpadded base64url characters. Creation retries a bounded number of times on its database uniqueness collision and otherwise fails safely. The secret component is exactly 32 separate CSPRNG bytes encoded as 43 canonical unpadded base64url characters. The non-secret display prefix is exactly `owl_client_v1.<public-key-id>`; it supports inventory, indexed lookup, safe log redaction, and secret-scanning patterns without disclosing any secret component. Alternate alphabets, padding, whitespace, duplicate separators, unknown versions, and non-canonical lengths are rejected before database access.
...
OwlAuth computes a purpose- and owner-bound HMAC-SHA-256 digest over a versioned length-delimited context containing the credential version, Project UUID, key UUID, public key ID, and 32 raw secret bytes. A dedicated versioned `OWLAUTH_CLIENT_KEY_DIGEST_KEY` key ring is available only to Control creation and Client verification composition. Every digest version referenced by an active key is retained in backup and every Client process. Client readiness fails when the authoritative active-key inventory references an unavailable verifier version.

Each Client-capable process has a stable configured process ID, one startup incarnation, and a bounded PostgreSQL readiness lease listing the exact digest versions it can verify. Control has the same configured required-Client roster. A create transaction may use its configured active digest version only when every required process ID has one current-incarnation, unexpired observation containing that version; an empty/missing/stale/mismatched roster fails closed. Client renewals do not authorize requests and expiry affects readiness/new creation, not verification by an already loaded process. Rollout installs the new retained version on Client first and waits for the complete roster before enabling Control creation; rollback keeps both versions. Retirement requires an authoritative zero-active-reference inventory after old keys are explicitly revoked. Loss or compromise cannot be repaired by rehashing: the operator restores the exact uncompromised version or revokes and reissues every affected key.
...
6. returns one private typed Client actor containing Project/key identity, never raw credential bytes.
...
| `GET /health`                                                                 | Client listener liveness/readiness                                         | no credential; no Project/key/topology disclosure                                      |

The Client API has no route for creating/updating/disabling/merging users, linking identities, changing metadata, revoking sessions, rotating keys, configuring providers, reading audits, replaying webhooks, or exporting a directory. Those remain Control responsibilities or are deliberately out of scope.

### Pagination and lookup

User listing uses deterministic ascending keyset ordering by immutable `(created_at, user_id)` and an opaque bounded cursor owned by the Client contract. Default and maximum limits are 50 and 100. The cursor is an ordinary bounded base64url encoding containing only the last ordering tuple and a format version; it is not a capability and does not need signing or expiry. Malformed cursors are rejected, and every resulting query remains Project-qualified.
...
The base Client user is a bounded Project-owned read model containing:
...
It excludes provider access/refresh tokens, provider client identifiers/secrets, raw claims/payloads, identity proof evidence, email lookup digests, `belongs_to`, protected-material IDs, Control revisions not needed by the read model, audits, session credentials, and another Application's private state.
...
Client uses a separate complete OpenAPI error vocabulary. Authentication failures are generic. An authenticated request may receive bounded `404`, conflict, rate-limit, or unavailable errors only for resources inside its own Project.

User/projection responses use `private, no-store`; key lifecycle responses use Control's no-store policy. Client responses never vary authority from cookies or browser origin. Request and response bounds are explicit, and Client does not follow or return cross-origin redirects.

Default authenticated Client limits are deliberately generous, deployment-configurable abuse guards rather than product quotas. They use resolved Project/key plus coarse source dimensions, allow ordinary backend bursts, and should not produce `429` during healthy expected SaaS traffic. Unknown/malformed credential attempts retain stricter source-based protection. OwlAuth does not add per-operation quota products or billing scopes.

Project disablement and key revocation affect the next authoritative request. Client-key authentication, user/email/projection reads, and token introspection consult PostgreSQL authority on every request and are never accepted from Redis or a process cache in v1. This keeps externally observable Client reads at the latest committed PostgreSQL state without inventing a directory-cache consistency protocol. Redis may still carry non-authoritative admission counters and post-commit invalidation hints for existing Runtime public caches; it never caches raw credentials, Authorization headers, email lookup bodies, introspection tokens, Client response bodies, or Client authentication decisions. Customer backends may cache minimized directory responses under their own product policy, but must not represent them as online revocation checks; Client introspection is the authoritative online token/session path.

## OpenAPI and implementation ownership

`crates/owlauth-types` owns three independent complete OpenAPI documents: Runtime, Client, and Control. Client DTOs and security schemes live in their own module and cannot import Control commands or Runtime credential requests. Documents are generated from reviewed Rust definitions and are not committed. The `owlauth-types` exporter is the build-time authority, and each server release attaches exact-version `owlauth-runtime-openapi.json`, `owlauth-client-openapi.json`, and `owlauth-control-openapi.json` artifacts generated from the same qualified source. No listener serves an OpenAPI route, so schema retrieval requires neither a Project key nor an exposed server endpoint.

`owlauth-server` implements the Client router and private application/repository ports. No published server repository, row, router, or private application error becomes public.

OwlAuth does not publish a Client API SDK in TypeScript, Python, Rust, or another language. Public docs show `curl` and OpenAPI generation examples without selecting a framework or generated-client vendor. Existing `sdks/*` consume only Runtime Project Auth OpenAPI and must reject any generated/import dependency on Client or Control types.
...
Client metrics use bounded route, outcome, Project-safe internal dimension, and key public ID only where cardinality policy permits. Logs and traces never contain `Authorization`, parsed secret components, token introspection input, email lookup-body plaintext, user profile values, or response bodies. Authentication denial does not reveal whether a public key ID exists.
...
- Console and CLI secret disposal, denial of browser persistence, real Chromium/Firefox page lifecycle behavior, and no Client credential support in Hosted/browser assets;
- independent Runtime, Client, and Control OpenAPI generation with clean regeneration and import-purity gates;
...
- real PostgreSQL Client API journeys and an independent security/architecture review have no unresolved supported P0/P1 finding.
