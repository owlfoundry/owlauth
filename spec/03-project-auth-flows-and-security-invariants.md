...
- The stable callback route is exactly `projects/{project_public_id}/auth/callback/{provider_key}` relative to the configured Runtime base. The path identifies the expected Project and provider configuration; callback parameters cannot select another Project/provider, interaction class, proof slot, or managed connection, and no second alias is accepted.
- Upstream state resolves exactly one persisted typed owner before any external I/O: an ordinary `login` transaction owned by this spec, an `identity_mutation` intent/slot owned by spec 11, or a `managed_reauthorization` interaction owned by spec 11. OwlAuth validates that owner's state digest/status, interaction browser binding, provider/Application assignment, Project, exact callback URI, expiry, and one-use transition. There is no fallback or conversion across classes.
- Before the external exchange, PostgreSQL atomically moves the selected typed owner from `provider_authorization_started` to `provider_exchange_in_progress`; concurrent callbacks cannot claim it. A callback that loses this claim returns only a bounded in-progress or terminal response and cannot mutate or terminalize the owner claimed by another callback. The remaining handoff/user-creation rules in this section apply only to `login`; mutation completion may persist only candidate/existing-identity evidence and one server-side receipt, while managed reauthorization may replace only the exact existing connection credential generation and bounded source result under spec 11.
- The OIDC adapter generates a fresh high-entropy nonce at the class-specific authorization-establishment transition: provider method selection for `login` or `identity_mutation`, and the browser/CSRF-bound fixed-provider start for `managed_reauthorization`. That transition atomically stores only the purpose-keyed digest and key version and immediately sends the exact bound nonce in its authorization redirect; a later request cannot regenerate or replace it. The validated ID token must contain one exact nonce match. Missing, duplicate, mismatched, or replayed nonce is terminal; caller input cannot supply it.
...
- Explicit or ambiguous exchange failure moves the exact typed owner to terminal `provider_exchange_failed`; login starts a new login, mutation starts a new intent/slot ceremony, and managed reauthorization starts a new reauthorization interaction instead of replaying the provider code.
...
- For `login`, an unknown verified identity creates a Project user and linked identity atomically, and the same provider identity in another Project creates/resolves an independent user. For `identity_mutation`, an unknown identity creates only short-term candidate evidence; durable identity creation/attachment waits for final Control confirmation. `managed_reauthorization` requires the exact frozen existing user, linked identity, provider configuration, and connection generation; a different or unknown issuer/subject terminalizes it without creating or moving anything.
- Matching email never silently links users. Link or merge requires explicit proof and Project-bound domain preconditions.
- A disabled or deleting Project, or a disabled user, cannot produce a handoff ticket, mutation receipt, or managed-reauthorization successor.
- Callback completion revalidates that the provider registration remains actively assigned to the captured Application; an assignment revision mismatch terminalizes the exact login, mutation slot, or managed reauthorization.
...
- No provider access/refresh token is returned downstream, stored in Project profile data, exposed through a generic provider API, or usable for Application-requested scopes. Provider credentials and full provider payloads never enter redirects, user projections, webhooks, audit context or ordinary logs.
...
- Project disablement, re-enablement, and accepted deletion each advance the Project security revision. Disabled and deleting Projects invalidate every Runtime operation; only a revision-checked disabled Project may be re-enabled, while deleting is terminal.
...
Logs, traces, metrics, errors, audit events, and OpenAPI examples never contain provider tokens, provider codes, handoff tickets, Project access tokens, refresh tokens, PKCE verifiers, cookies, provider secrets, Project server keys, the deployment operator API key, private keys, full callback URLs, or complete user profiles. Ordinary agent context follows the same rule. The sole model-context exception is an explicitly invoked full-authority remote Control MCP operation governed by [the MCP boundary](07-cli-and-mcp-boundaries.md): only its declared Control arguments and reviewed response may become model-visible, including write-only provider/SMTP/webhook configuration secrets, a one-time Project server-key credential, and returned identity presentation fields. The operator key, stored provider/session/token material, signing private keys, and undeclared repository data remain excluded. Audit events record Project, Application, stable user/target references where authorized, action, outcome, reason class, and correlation without recoverable credentials.
