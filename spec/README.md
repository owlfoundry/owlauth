...
The server deliberately has one deployment-level operator trust model rather than organizations, memberships, tenant RBAC, or billing. Separately, each Project may issue bounded read-only server keys for its customer backend Server API; those keys never authorize Control and do not turn OwlAuth into a multi-tenant SaaS control system.
...
| [`04-storage-and-migrations.md`](04-storage-and-migrations.md)                                                               | PostgreSQL authority, project-scoped data model, transactions, migrations, and recovery                                             |
| [`05-http-contract-and-surface-boundaries.md`](05-http-contract-and-surface-boundaries.md)                                   | Runtime, Server API, and Control HTTP surfaces, endpoint isolation, DTO ownership, and error contracts                              |
...
| [`13-server-api-and-project-server-keys.md`](13-server-api-and-project-server-keys.md)                                       | Project server-key lifecycle, customer-backend Server API on Auth, read models, introspection, and OpenAPI boundary                 |
...
| Public Runtime, Server API, and Control HTTP DTOs and OpenAPI definitions                                                           | Rust definitions in `crates/owlauth-types`                                                                                       |
| PostgreSQL schema and migration assets                                                                                              | `crates/owlauth-server/migrations/`                                                                                              |
| Auth and Control composition/listeners, surface-specific adapters, persistence, embedded hosted web surfaces, and operational ports | `crates/owlauth-server`                                                                                                          |
...
04. OwlAuth has one shared application/domain core. Runtime HTTP, Server API HTTP, Control HTTP, the CLI's Control adapter, and remote HTTP MCP do not own alternate server business rules.
05. PostgreSQL is the authoritative transactional store for identity, handoff consumption, session validity, refresh rotation, revocation, Project status, and key state.
06. Auth and Control use distinct listeners. Within Auth, Runtime Project Auth and Server API use distinct routers, state, authentication policies, PostgreSQL pools, readiness inputs, and OpenAPI contracts.
...
09. Browser applications receive only public Project/Application configuration. Project server keys, provider secrets, the operator API key, refresh-token digests, and private keys never enter public configuration, browser assets, or logs. Runtime never accepts operator or Project server keys.
...
11. The server has one Rust domain/application implementation and one official binary/container artifact. `all`, `auth`, and `control` are the only composition modes, not independent domain implementations. A small published `owlauth-key-provider` SPI and narrow public server composition API permit statically linked custom binaries without a runtime plugin ABI or bundled KMS implementation.
...
16. Customer backends consume the independent Server OpenAPI with a Project server key and own generated clients or SaaS/BFF wrappers. OwlAuth does not publish a Server API SDK; existing language SDKs remain secret-free Runtime Project Auth clients.

## Normative language

`MUST`, `MUST NOT`, `SHOULD`, and `MAY` are normative design terms. “Runtime” means the browser-facing Project Auth surface within Auth. “Server API” means the Project-server-key-authenticated customer-backend surface within Auth. “Control” means the independently bound deployment administrative endpoint. “Shared core” means the application services, domain model, and ports used by all surfaces.
