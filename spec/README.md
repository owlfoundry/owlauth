...
The server deliberately has a single deployment-level operator trust model rather than organizations, memberships, tenant RBAC, SaaS API keys, or billing. The separate multi-tenant service architecture is specified in [`spec/saas/`](saas/); it is not part of the `owlauth-server` boundary defined here.
...
| Public remote administration client | one `crates/owlauth-cli` executable whose profiles discover and pin `owlauth-server` or `owlauth-saas`; this root spec owns server dispatch and [`spec/saas/`](saas/) owns SaaS dispatch |
...
1. One OwlAuth deployment is one administrative trust domain. Its Control API accepts only the deployment's `OWLAUTH_CONTROL_API_KEY`; a valid key has full Control authority. It may contain many isolated Projects but is not itself a multi-tenant organization or RBAC system.
...
4. OwlAuth has one shared application/domain core. Runtime HTTP, Control HTTP, the CLI's `server` adapter, and self-hosted HTTP MCP do not own alternate server business rules. The CLI's `saas` adapter and SaaS MCP enter SaaS application services instead of this core.
...
8. Provider callbacks and application redirects are distinct URL classes and both use exact registered values. Login state, handoff tickets, refresh tokens, and sessions have explicit binding and replay semantics.
9. Browser applications receive only public Project/Application configuration. Provider secrets, the operator API key, refresh-token digests, and private keys never enter public configuration or logs. Runtime never accepts the operator API key.
...
15. One `owlauth` executable uses endpoint-discovered profiles pinned to product, instance, authority, API base, and credential class with no user-configured product type, authenticated probing/fallback, or credential crossover. Self-hosted and SaaS MCP are separate remote HTTP servers that self-describe through the protocol; no CLI/plugin bundles a local MCP process.
