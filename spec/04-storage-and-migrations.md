...
PostgreSQL is OwlAuth's sole authoritative transactional store. Projects, Applications, redirect/origin registrations, provider configuration, Project client-key commitments, Project users, linked/email identities, managed provider connections, login/email challenges, handoff tickets, sessions, refresh families, user revisions/Application projections, SMTP and webhook configuration, durable delivery outboxes, revocation, policy, Project key metadata, provisioning operations, and audit events are correct only when established by committed PostgreSQL state. Raw Project client credentials are deliberately excluded after their one-time create response, and the deployment operator API key exists only in Control process configuration.
...
Runtime, Client, and Control use one schema and one shared core. There are no per-plane databases, cross-plane RPC transactions, message brokers, or distributed commits.
...
- A Project client-key row belongs directly to one Project and has immutable key UUID, public key ID, digest/key version, and Project ownership.
...
- Client process-incarnation and digest-readiness rows contain only stable process ID, startup incarnation, exact supported digest versions, check/lease times, and bounded safe failure class. They authorize neither a request nor a key; Control uses the complete configured roster only to prevent creation under a version the Client fleet cannot yet verify.
- V1 Client user/email/projection/introspection reads query PostgreSQL on every request and have no Redis/process response cache.

### Provider configurations

- `(project_id, provider_key)` is unique. A provider configuration represents one upstream OAuth/OIDC client registration with canonical issuer/kind, client ID, callback identity, monotonic secret generation, and one same-Project protected-secret material ID. Secret replacement advances that generation and owner/material tuple atomically; ordinary metadata revision cannot retarget existing in-flight work to replacement material.
...
- The upgrade bridge treats the validated legacy `OWLAUTH_PROVIDER_ALLOWED_ORIGINS` value only as migration input: before admitting Custom OIDC, it writes that canonical set as `exact_origins` for every existing Project missing policy, then records completion and stops consulting the process value. Project creation concurrently uses the target `allow_all` default through the same durable initialization constraint/trigger. A missing policy row fails only Custom OIDC readiness closed. Existing generic `oidc` rows at reserved Google/GitHub issuer roots become unavailable until recreated through the matching named profile; the bridge never silently reclassifies them or preserves generic behavior at a reserved root.
...
- Plaintext secret bytes are absent from PostgreSQL and Redis. PostgreSQL contains only the bounded opaque authenticated envelope and safe keyed fingerprint in its protected-material record; neither value nor plaintext appears in public Project config, Runtime responses, audit events, or logs.
...
- distributed Runtime, Client, and Control rate counters and short-lived abuse signals;
...
Control commits a mutation and audit event to PostgreSQL first, then publishes a best-effort invalidation carrying Project, entity, identifier, and new revision. Runtime treats invalidation as a latency optimization for its permitted public caches. V1 Client directory/email/projection/introspection responses are not Redis-cached and query PostgreSQL on every request.
...
Every build before the current post-initial series is pre-alpha and carries no binary/schema compatibility promise. Operators stop all older processes before applying it; the first committed post-initial migration advances the floor to the current binary schema level, so an older binary cannot restart against either the complete schema or a partial successful prefix. No old-writer compatibility trigger or concurrent rolling overlap is retained. After a migration history is published, its bytes become append-only; any future rolling release must explicitly prove its N/N-1 expand/migrate/switch/contract overlap and advance the floor before contraction rather than inferring compatibility from additive-looking SQL.

The TS-003 encrypted-file-to-protected-material transition is an explicit bridge, not an ordinary startup SQL backfill. Its additive schema includes a singleton revisioned custody-cutover authority with states `legacy`, `importing`, and `protected`. The bridge binary reads both formats and initially writes legacy format. Before entering `importing`, every pre-bridge process is drained because it cannot honor the authority row. Every bridge writer captures the authority revision before any file/seal work and conditionally rechecks it in the final owner/operation transaction; a writer prepared under `legacy` cannot commit a legacy-only owner after the authority changes. The bounded importer is a migration-only composition that temporarily receives both legacy roots and the new software custody root, exposes no business listener or generic export API, and is resumable/idempotent by immutable legacy owner/material identity. Ordinary serving composition does not retain both root sets after contraction. The final switch locks the authority row, proves that every live/retained signer and provider/SMTP/webhook generation has exactly one verified protected-material successor and no unresolved import, then atomically enters `protected`. A losing concurrent legacy writer retries through protected sealing. No dual-write is treated as authority. Legacy columns, files, roots, and bridge code contract only after the rollback window closes and a zero-reference inventory is proven.

## Durability and recovery model

A recoverable OwlAuth state consists of:

- a transactionally consistent PostgreSQL backup, including Application bindings/projections, projection-expansion cursors, immutable events/deliveries, protected-material envelopes/handles and their lifecycle tombstones, and committed attempt state;
...
- every Project client-key digest root/version still referenced by an active key, supplied independently to Control/Client as their narrow issuer/verifier capabilities;
