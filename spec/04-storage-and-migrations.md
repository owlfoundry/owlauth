...
- Auth process-incarnation and digest-readiness rows contain only stable process ID, startup incarnation, exact supported digest versions, check/lease times, and bounded safe failure class. They authorize neither a request nor a key; Control uses the complete configured roster only to prevent creation under a version the Client fleet cannot yet verify.
...
- `projection_email_key_authority` is the singleton PostgreSQL authority for a positive revision, one current write version, at most 16 accepted versions, an optional staged cutover target, and a separately authorized retirement. `projection_email_runtime_observations` records the exact current Auth process incarnation, observed authority revision, bounded readable versions, and lease expiry. Callers supply only strictly positive bounded lease/retention durations: PostgreSQL `clock_timestamp()` authors observation, expiry, staging, cutover, authorization, retirement/update timestamps and evaluates lease/retention elapsed checks, so process clock skew cannot extend authority or accelerate retirement. Every required Auth process must observe a staged version under its current incarnation before cutover. Ordinary confirmation checks the already activated write version locally and performs no Runtime RPC. A referenced version cannot retire; after references reach zero, a separate retirement authorization must survive the configured propagation-retention interval before removal from accepted versions. Each transition is revision-CAS and restart-safe.
...
- distributed Runtime, Server API, and Control rate counters and short-lived abuse signals;
