...
| Mode      | Runtime listener | Client listener | Control listener | Shared core               | PostgreSQL schema |
| --------- | ---------------- | --------------- | ---------------- | ------------------------- | ----------------- |
| `all`     | enabled          | enabled         | enabled          | one in-process instance   | shared            |
| `runtime` | enabled          | absent          | absent           | Runtime capabilities only | shared            |
| `client`  | absent           | enabled         | absent           | Client capabilities only  | shared            |
| `control` | absent           | absent          | enabled          | Control capabilities only | shared            |
...
Configuration has one precedence model, rejects unknown fields, and separates global, Runtime, Client, Control, PostgreSQL serving/migration, Redis, key-provider/software-custody, data-protection, and telemetry sections.

### Global fields

- immutable deployment external Runtime, Client, and Control base URLs; any shared external origin uses three disjoint non-root prefixes while internal listeners remain separate;
...
### Client listener fields

- distinct internal bind address and configured external Client base URL;
- TLS/trusted-proxy mode, JSON-only response policy, no browser CORS/cookies/HTML/redirects;
- generous deployment-configurable authenticated Project/key limits plus stricter malformed/unknown-key source protection;
- independent connection, concurrency, body, timeout, serving-pool, and readiness bounds;
- dedicated active/retained `OWLAUTH_CLIENT_KEY_DIGEST_KEY` ring available as generation/digest capability only in Control and candidate-verification capability only in Client;
- stable Client process ID, required Client process roster shared by Client/Control, and bounded PostgreSQL digest-readiness lease/renewal configuration used only for verifier-first version rollout and readiness.
...
`OWLAUTH_CONTROL_API_KEY` is required when mode is `control` or `all`; startup fails before binding Control, Console business routes, or an enabled HTTP MCP endpoint if it is absent or does not match the canonical format below. Modes `runtime` and `client` do not require or load it. No configuration field defines additional keys, operator identities, permissions, or Control sessions. Any built-in Control UI invokes the same Control API using the same Bearer key and creates no server-side login/session model.

### Infrastructure fields

- one PostgreSQL serving server/database target, with optional plane-specific login credential references, independent pool bounds, DDL-free roles, bounded connect/acquire waits, and one validated 10-60000 ms database-enforced `lock_timeout` applied and verified on every Runtime, Client, and Control physical connection;
...
- a stable Client process identity and complete required Client roster, with current-incarnation digest-version readiness observations and bounded leases; Client and Control configurations must name the same non-empty roster, and each Client ID appears exactly once;
- generic Runtime data-protection/digest active and retained versions loaded only by Runtime-capable processes;
- a dedicated versioned identity-mutation evidence digest/protection ring required in every Runtime, Client, Control, or combined serving mode, with every active and retained root distinct from all other authorities; composition exposes only the Runtime producer/receipt or Control verifier/decrypt facade and never loads generic Runtime roots in Control;
- a separate active/retained managed-reauthorization target ring whose narrow issuer capability is loaded by Control and verifier capability by Runtime, and whose keys are distinct from every generic Runtime, Runtime admission, Client-key digest, and managed-credential root;
- a dedicated active/retained Project client-key digest ring whose narrow generator/digester is loaded by Control and candidate verifier by Client; it is distinct from Runtime, admission, email, identity-mutation, managed-reauthorization, managed-credential, projection, and custody roots;
- the dedicated AEAD-only `OWLAUTH_PROJECTION_EMAIL_*` active/retained ring, distinct from generic Runtime, durable email-identity, managed credential/reauthorization, admission, and signing roots; it has no digest key because the public canonical projection digest is unkeyed SHA-256. Control receives only exact-context encrypt/decrypt through the transaction projection materializer, while Runtime receives exact-context projection read/write. `OWLAUTH_PROJECTION_EMAIL_CUTOVER_VERSION` and `OWLAUTH_PROJECTION_EMAIL_RETIRE_VERSION` are mutually exclusive rollout authorizations checked against PostgreSQL authority and required current-incarnation Runtime observations;
...
- during the bounded provider-policy upgrade bridge only, the validated legacy `OWLAUTH_PROVIDER_ALLOWED_ORIGINS` set is required as migration input for existing Projects and is durably copied to each missing Project row as `exact_origins` before Custom OIDC becomes ready; after bridge completion it is rejected/ignored as authority and may be removed, while concurrently created Projects use the target `allow_all` default;
...
Project client-key digest rollout is verifier-first. Operators install the new retained version on every required Client replica and wait for version-specific readiness inventory before Control may make it active for creation. Rollback keeps the prior and new versions readable. Every version referenced by an active key remains configured, backed up, and ready; retirement is rejected until PostgreSQL proves zero active references. A missing referenced version keeps Client unready. A compromised version is not rehashed or recovered from stored digests: affected keys are explicitly revoked, reissued under an uncompromised version, and redeployed before the old version retires.
...
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Control              | Project key metadata/public JWK, lifecycle command, signing-key provisioner, configuration-secret sealer, purpose-limited managed-reauthorization target issuer, identity-mutation evidence verifier/decrypt facade                                                   | configuration-secret opener, Runtime signer, generic Runtime digest/protection roots, evidence producer/receipt authority, managed credentials, exportable private key, or secret bytes in DTOs |
...
Runtime readiness requires compatible PostgreSQL state, correctly composed provider/data-protection capabilities, resolvable non-erased material for served Projects, and a safe response to Redis availability. A wrong bundled custody root is detected by authenticated opening/signing checks and never causes fallback. Project-specific upstream/key-provider/SMTP/protector failures close only the affected provider, signing, email-auth, PII, or managed-sync capability without exposing detail globally. A missing/mismatched deployment-default SMTP registry generation keeps default-backed email challenge admission/claims unready; provider login and active sessions may remain available when only asynchronous profile sync/webhook delivery is degraded. Readiness never discards a pending projection cursor/event or silently substitutes a missing key/secret generation. Control readiness requires PostgreSQL and a valid loaded `OWLAUTH_CONTROL_API_KEY`; key/provider/secret mutations can return operation-specific dependency failure.
...
PostgreSQL/Redis pools, upstream-provider exchanges, key-provider operations, and per-listener/Project concurrency are bounded independently. Backpressure rejects before unbounded queues form. Detailed dependency outcomes are defined by spec 08.
