...
Configuration has one precedence model, rejects unknown fields, and separates global, Auth transport, Runtime surface, Server API surface, Control, PostgreSQL serving/migration, Redis, key-provider/software-custody, data-protection, and telemetry sections.
...
- one bind address, external Auth base URL, TLS/trusted-proxy mode, connection budget, request-shape bounds, deadlines, process ID/incarnation, health endpoint, and aggregate readiness endpoint;
- Runtime-only cookie security, Project namespace behavior, public configuration, hosted authentication UI, JWKS cache bounds, exact browser CORS enforcement, and Project/Application admission policy;
- Server API-only JSON response policy with no browser CORS/cookies/HTML/redirects, Project server-key authentication, generous authenticated Project/key limits, and stricter malformed/unknown-key source protection;
- separate Runtime and Server API routers, state, admission services, authorization middleware, metrics, PostgreSQL serving pools, pool bounds, and readiness inputs despite the shared transport;
...
- one stable Auth process ID and required Auth roster shared by Auth/Control, with bounded PostgreSQL Runtime publication and Server digest-readiness leases tied to the same current process incarnation.
...
- a stable independent 32-byte Auth admission digest root that remains unchanged across Runtime protection-key rotations, an optional admission Redis URL (`redis` or `rediss` only), a bounded deployment-unique namespace derived from the required instance ID by default, a 10-2000 ms operation deadline, and a configured maximum Auth process count from the required roster size through 64;
...
- one stable Auth process identity and complete required Auth roster, shared by Runtime publication and Server digest readiness, plus publication-lease lifetime and minimum key-propagation interval; Auth and Control configurations name the same non-empty roster, and each Auth ID appears exactly once;
...
- a separate active/retained managed-reauthorization target ring whose narrow issuer capability is loaded by Control and verifier capability by Runtime, and whose keys are distinct from every generic Runtime, Auth admission, Server-key digest, and managed-credential root;
- a dedicated active/retained Project server-key digest ring whose narrow generator/digester is loaded by Control and candidate verifier by the Server API surface; it is distinct from Runtime, admission, email, identity-mutation, managed-reauthorization, managed-credential, projection, and custody roots;
...
Project server-key digest rollout is verifier-first. Operators install the new retained version on every required Auth replica and wait for version-specific Server readiness inventory before Control may make it active for creation. Rollback keeps the prior and new versions readable. Every version referenced by an active key remains configured, backed up, and ready; retirement is rejected until PostgreSQL proves zero active references. A missing referenced version keeps Auth unready. A compromised version is not rehashed or recovered from stored digests: affected keys are explicitly revoked, reissued under an uncompromised version, and redeployed before the old version retires.

Issuer, callback, redirect, and admission-source decisions never derive from arbitrary `Host`, `Forwarded`, or `X-Forwarded-*`. Auth and Control use only the direct transport peer, and this release exposes no trusted-forwarding mode.
...
Auth and Control bind plain-HTTP IP sockets and require an operator-owned TLS terminator for every production external origin. Control SHOULD bind privately; network isolation supplements application authentication. Auth serves the Runtime Hosted Authentication UI and backend-only Server API; Control serves the Management Console. Distinct external origins are recommended. An explicitly configured shared origin uses disjoint non-root Auth/Control paths, contains Runtime cookies to the Auth base, registers no service workers, applies restrictive opener policy, and deliberately shares one browser/XSS boundary; Control retains a separate listener, while Runtime and Server API retain internal router, credential, response, admission, and pool boundaries as defined by spec 09.

Each listener applies connection/header/body/URI bounds, direct-peer client-address derivation, correlation, surface rate/concurrency controls, authentication where applicable, and safe response headers before expensive work. Server API admission defaults are deliberately generous and configurable for normal backend traffic; they remain abuse protection, not a quota/billing product. Server-key authentication, directory/email/projection reads, and introspection query PostgreSQL authority on every v1 request. The current release uses optional Redis only for non-authoritative distributed Runtime and Server API admission counters; bounded process-local caches remain non-authoritative and no Redis public-configuration/JWKS cache or invalidation bus is implemented.
...
Each ready Auth process loads the current Project key-ring revision before serving that Project's JWKS/signing path and writes a short PostgreSQL publication lease containing instance identity, loaded revision, the time that revision was first loaded, and lease expiry. Lease renewal extends expiry without resetting the revision-loaded time. Load balancers route only ready Auth processes; a new/returning process cannot serve signing/JWKS while stale.
