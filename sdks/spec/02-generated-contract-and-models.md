...
cargo run --locked --package owlauth-types --bin export-openapi -- \
  server <temporary-build-path>/server-openapi.json
cargo run --locked --package owlauth-types --bin export-openapi -- \
...
- Server user/projection/introspection DTOs intended for customer-owned generated backend clients;
...
Default SDK generation consumes Runtime only. It must not expose Server API/Control endpoints, Project server keys, management credentials/scopes, server storage models, provider payloads, key references, or internal health detail merely because one server binary contains those surfaces. OwlAuth exports the complete Server OpenAPI for users but does not generate or publish an official Server API SDK.
...
- Runtime/Server API/Control surface leakage.
...
- The policy and normalized default SDK surface contain only the reviewed Runtime Project Auth operations and no Server API or Control authority.
