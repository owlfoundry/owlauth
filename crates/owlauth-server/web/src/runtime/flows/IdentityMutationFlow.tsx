...
function stateLabel(state: IdentityMutationSlotState): string {
  switch (state) {
    case "unselected":
      return "Not started";
    ...
    case "email_address_entry":
      return "Waiting for an email address";
    case "email_challenge_pending":
      return "Waiting for the newest email proof";
    case "proved":
        ...
        {intent.slots.map((slot) => {
          ...
          return (
            <li key={slot.id}>
              ...
              <p>
                Verification method:{" "}
                {slot.method_kind === "provider" ? "Identity provider" : "Email"}. Status:{" "}
