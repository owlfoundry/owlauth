...
interface IdentityMutationPanelProps {
...
function safeHostedTarget(value: string | null | undefined): string | null {
...
export function IdentityMutationPanel({
  session,
  project,
  selectedUser,
  users,
  ...
  async function cancelIntent() {
    if (active === null || !window.confirm("Cancel this exact identity proof intent?")) return;
    setPending(true);
    try {
      const result = await session.client.POST(
  ...
  return (
    ...
    >
      <h5 id="identity-mutation-console-heading">Identity link, unlink, and merge</h5>
      <p>
      ...
      <form className={styles["form"]} onSubmit={(event) => void createIntent(event)}>
        <h6>Create an immutable proof plan</h6>
      ...
      {active === null ? null : (
        <article className={styles["panel"]} aria-labelledby="active-identity-intent">
          <h6 id="active-identity-intent">Identity mutation intent</h6>
          <p>
          ...
          active.intent.operation_kind === active.plan.operation_kind ? (
            ...
            >
              <h6 id="final-identity-confirmation">Final ownership confirmation</h6>
              <p>
...
function ExactPlanReview({ plan }: { readonly plan: CreateIdentityMutationIntentRequest }) {
  return (
    <section aria-labelledby="exact-plan-review" className={styles["planReview"]}>
      <h6 id="exact-plan-review">Exact immutable create-time plan</h6>
      <p>
