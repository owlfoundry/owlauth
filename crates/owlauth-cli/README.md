...
Profile storage accepts only the current `owlauth-server`/`operator-api-key` schema. Any unknown product, credential class, crossed pair, field, or schema version fails closed as malformed profile storage. Compatible `api_versions` additions and MCP enable/disable changes are non-authoritative capability-cache updates, not identity changes; `profile check` and `profile use` refresh that cache without changing the pin or credential reference.
